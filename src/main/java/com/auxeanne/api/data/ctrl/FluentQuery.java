...
import com.auxeanne.api.data.db.RecordLink;
import com.auxeanne.api.data.db.RecordPath;
import com.auxeanne.api.data.db.RecordWrapper;
...
public class FluentQuery {
    ...
    static public class Builder<T extends Record> implements SelectBuilder<T>, QueryBuilder<T>, LinkBuilder<T>, LinkAttributeBuilder<T>, PathBuilder<T>, ExtendedQuery<T>, SortBuilder<T>, SortLinkBuilder<T> {
        ...
        private final HashMap<String, List<String>> matchMap = new HashMap<>();
        ...
        private void initRecordQuery() {
            ...
            predicateList.add(cb.equal(recordRoot.get("recordType").get("code"), referenceClass.getName()));
            //-- adding match filter
            applyExtendedQuery(recordRoot, predicateList);
        }
        ...
        public ExtendedQuery<T> fieldEqualTo(String key, String value) {
            matchMap.put(key, Arrays.asList(value));
            return this;
        }

        @Override
        public ExtendedQuery<T> fieldIn(String key, String... values) {
            matchMap.put(key, Arrays.asList(values));
            return this;
        }
        ...
        private void applyExtendedQuery(Path recordPath, List<Predicate> predicateList) {
            applyMatchQuery(recordPath, predicateList);
            applyIndexQuery(recordPath, predicateList);
        }

        private void applyMatchQuery(Path recordPath, List<Predicate> predicateList) {
            Iterator<String> keyIterator = matchMap.keySet().iterator();
            while (keyIterator.hasNext()) {
                String key = keyIterator.next();
                List<String> values = matchMap.get(key);
        ...
        private void applyIndexQuery(Path recordPath, List<Predicate> predicateList) {
        ...
        private void initLinkQuery() {
            ...
            //-- forcing additional inner join for better performance
            predicateList.add(cb.equal(linkRoot.get("reference").get("recordType").get("code"), referenceClass.getName()));
            //-- adding match and index filters
            applyExtendedQuery(linkRoot.get("reference"), predicateList);
        }
        ...
        private <T> void connectPath(boolean limitPath, boolean and, String source, String target, Record... records) {
            ...
            pathTarget = target;
            //-- forcing additional inner join for better performance
            predicateList.add(cb.equal(pathRoot.get(target).get("recordType").get("code"), referenceClass.getName()));
            //-- adding match and index filter
            applyExtendedQuery(pathRoot.get(target), predicateList);
            //-- adding predicates
            if (and) {
        ...
        private <T> void connectAll(boolean limitPath, Class<T> c, Root<T> root, String source, String target, Record... records) {
            ...
            //predicateList.add(cb.equal(root.get("target").get("recordType").get("code"), referenceClass.getName())); // using name works better than using id by x40 , cannot explain yet???? SQL shows additional inner join
